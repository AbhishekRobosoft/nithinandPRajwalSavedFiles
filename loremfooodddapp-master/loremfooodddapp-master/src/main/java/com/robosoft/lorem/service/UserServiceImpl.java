...
public class UserServiceImpl implements UserService, UserDetailsService {
    ...
    @Override
    public boolean checkOrderDetials(Payment payment, int cartId)
    {
        String email = getUserNameFromToken();
        int id = jdbcTemplate.queryForObject("select userId from user where emailId=?", Integer.class, new Object[]{email});
        ...
        if(!this.checkIfScheduledAtleastAnHourAfter(payment.getScheduleDate(),payment.getScheduleTime()))
            throw new InvalidScheduledDateTimeException("Payment Failed");
        try
        {
            ...
            if(amount!=payment.getAmount())
                return false;
            float taxAmount=payment.getAmount()*(5/100);
            ...
            if(payment.getGrandTotal()!=totalAmount)
                return false;
            if(!checkCvv(payment.getCardNo(),payment.getCvv()))
                return false;
        }
        catch (Exception e)
        {
    ...
    public boolean checkCvv(String cardNo, String Cvv)
    {
        String cvv = jdbcTemplate.queryForObject("select cvv from card where cardNo=?", String.class, new Object[]{cardNo});
